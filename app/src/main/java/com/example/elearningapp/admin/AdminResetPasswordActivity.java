    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn_reset.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (email.equals("")){
                    Toast.makeText(AdminResetPasswordActivity.this, "Email is empty", Toast.LENGTH_LONG).show();
                }else{
                    firebaseAuth.sendPasswordResetEmail(email).addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
                        public void onComplete(@NonNull Task<Void> task) {
                            if (task.isSuccessful()){
                                Toast.makeText(AdminResetPasswordActivity.this, "Please Check Your Email", Toast.LENGTH_LONG).show();
                                startActivity(new Intent(AdminResetPasswordActivity.this, AdminLogin.class));
                            }else{
                                String error = task.getException().getMessage();
                                Toast.makeText(AdminResetPasswordActivity.this, error, Toast.LENGTH_LONG).show();
                            }
                        }
                    });
                }
